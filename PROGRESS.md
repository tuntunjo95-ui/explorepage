...
- Plaza 互动栏的点赞/评论统一为 1.8px 深灰线性 SVG（22px 视觉尺寸、44px 点击区）；点赞、评论、转发默认统一为 `#38383e`，点赞后爱心以红色 `#ef4056` 填充并播放 280ms 轻量弹跳动效；个人主页同步同款图标。
- 纠正加载交互：移除顶部下拉刷新；Explore 首批展示12条帖子，浏览至距离列表底部约半屏时自动预加载下一批8条，加载期间仅在列表末尾显示2条轻量骨架屏，已浏览内容不被替换，并兼容 `prefers-reduced-motion`。
- 纯色 QA 图片并入新版统一媒体规则：横图铺满内容栏且最高235px，方图最大300px，竖图最高375px并按原比例收窄，多图保持216–235px响应式高度。
- Explore 图片比例演示之间穿插7条纯文字帖，方便同时评审图文与纯文字信息密度。

## 2026-08-25（Codex·Plaza 帖子时间短格式）
- 广场帖子作者行与帖子详情顶部的发布时间改为 Threads 式短格式：`now / just now → 1s`、`18 seconds ago → 18s`、`1 minute ago → 1m`，并支持 `h/d/w/mo/y`；钟点时间保持原样。
- 完整原时间保留在 `title` 与 `aria-label`，评论时间、消息列表时间和系统状态栏不受影响。
- 393×852 首屏截图确认 `18s / 1m / 2m` 与 Follow/Chat 对齐正常；`npm run build` 通过。全量 lint 仍被仓库既有的 5 个错误和 1 个 warning 阻断，本轮未新增相关问题。
- 本轮仅做本地改动，未提交、未发布。
- 新增一条无帖子文案的纯图片帖示例：正文节点不渲染，图片在作者栏下保留 8px 间距，互动栏与评论预览照常展示。
- 纯图片帖作者头像由自带白色遮挡的 `dede_avatar` 换为完整干净的 `calista` 头像。
- 修正作者栏右上角 More 的光学垂直居中：保持 44×44px 点击热区不变，仅将三点字形下移，使其与名字/时间行上下居中；同时实机截取列表末尾预加载骨架样式用于评审。

## 2026-08-28（Codex·Plaza Follow / Chat 按钮对比版本，本地预览）
- 在 Plaza 同一页面新增 `Actions` 对比器，保留 `Current` 标准版，并加入 `No Follow / Chat` 与 `No Chat` 两个版本；选择同步写入 `actions` 查询参数。
- `No Follow / Chat`：普通帖、群聊帖与帖子详情均不渲染 Follow、关注态或 Chat 控件，头像/群聊水波纹与 More 保持原布局。
- `No Chat`：保留 Follow 与关注态，只移除 Chat；帖子详情中未关注仍可 Follow，已关注时不补其他新按钮。
- 393×852 与 360×800 均完成浏览器截图和 DOM 检查：无横向溢出、无 More/作者信息重叠、无坏图或控制台错误；`npm run build` 通过。
- 本轮按用户确认进入线上发布；生产构建与线上回读结果以本次发布记录为准。

## 2026-08-31（Codex·Plaza 互动栏三等分对比，本地预览）
- 在同一 Plaza 页面新增独立 `Metrics` 对比器，保留 `Current`，新增 `Equal thirds`；选择同步写入 `metrics` 查询参数，不影响已有 `Actions` 对比器。
- 三等分版以帖子最大内容宽度 311px 为上限，将点赞、评论、分享均分为三列；小屏随可用内容区收缩，各操作组在分区内居中且继续保留 44px 点击热区。
- 信息流与帖子详情共用该规则；点赞态、数字变化、评论跳转、分享反馈以及匿名深色模式行为均不变。
- 浏览器实测：360×800 为 284÷3，393×852 与 430×932 为 311÷3；三列宽差均为 0、无横向溢出。`npm run build` 通过；截图与检查记录见 `docs/evidence/interaction-third-layout.md`。
- 本轮已按用户确认进入源码提交与线上发布；生产构建和线上回读以本次发布结果为准。
- 依据评审反馈修正三等分的视觉对齐：三列点击区继续等宽，但点赞视觉组贴齐图片左边缘、评论保持内容区正中、分享视觉组贴齐图片右边缘；互动栏宽度直接复用单图最大展示宽度变量，360px 小屏也与实际图片边界完全一致。
